#!/bin/bash
# ICMP Flood from Kali → Linux target
# Triggers: ICMP flood SCREEN (threshold: 1000 pps)
...
echo "Triggers : ICMP flood SCREEN (threshold: 1000 pps)"
echo ""
echo "Starting ICMP flood..."

sshpass -p 'Juniper!1' ssh $SSH_OPTS jcluser@$KALI \
    "timeout ${DURATION} sudo /usr/sbin/hping3 -1 --flood ${TARGET} 2>&1 | tail -5" 2>/dev/null

echo ""
echo "Attack complete. Check monitor.sh for ICMP flood SCREEN hit counters."
